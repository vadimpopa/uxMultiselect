app.run(['$templateCache',
    function($templateCache) {
        ...
        $templateCache.put('ux-multiselect.html',
    	    ['<div data-outside-click="onOutsideClick()">',
                ...
                '<div class="ux-multiselect-selector" data-ng-show="isOpen()">',
                    '<ul class="ux-multiselect-ul" data-ux-delegate="click | li | addItem(item)">',
                        '<li class="ux-multiselect-selector-li" data-ng-repeat="item in matches | limitTo: limitFilter"',
                            ...
                            '<div data-ux-multiselect-match data-template-url="\'{{itemTemplateUrl}}\'"></div>',
                        '</li>',
                    '</ul>',
        ...
        $templateCache.put("ux-multiselect-match.html",
            '<span data-ng-bind-html="\'{{item[displayfield]}}\' | uxMultiselectHighlight: query | tgTrustedHtml "></span>');
}]);

app.directive('uxMultiselect', ['$templateCache', '$document', '$q', '$timeout', '$parse', function($templateCache, $document, $q, $timeout, $parse) {
	    return {
		    ...
		    link: function (scope, element, attrs) {
                ...
                scope.itemTemplateUrl = angular.isDefined(attrs.typeaheadTemplateUrl) ? attrs.typeaheadTemplateUrl : "ux-multiselect-match.html";

			    var isOnDocClickAttached = false,
                    changeCallbackExpression,
     			    timeoutPromise,
     			    waitTime = 600;

                if(attrs.outputChangeCallback) {
                    changeCallbackExpression = $parse(attrs.outputChangeCallback);
                }
			    ...
			    function getMatchesAsync(query) {
			    ...
			    scope.setFocus = function(event){
				    var inpEl = element.find("input");

				    if(!inpEl.is(event.target)) {
			    ...
			    scope.addItem = function addItem(item) {
                    ...
                    if(changeCallbackExpression) {
                        changeCallbackExpression(scope.$parent);
                    }
			    ...
			    scope.removeItem = function removeItem(position) {
				    scope.output.splice(position, 1);
    ...
    .directive('uxMultiselectMatch', ['$http', '$templateCache', '$compile', '$parse', function ($http, $templateCache, $compile, $parse) {
        return {
            restrict:'EA',
            link:function (scope, element, attrs) {
                var tplUrl = $parse(attrs.templateUrl)(scope.$parent) || 'ux-multiselect-match.html';
    ...
    .directive("uxDelegate", ['$parse', function($parse) {
